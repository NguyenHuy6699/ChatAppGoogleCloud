...
public interface ChatRepository extends JpaRepository<ChatMessage, Long> {
	...
	@Query(value = "SELECT * FROM message WHERE (sender_username = :userA AND receiver_username = :userB) OR (sender_username = :userB AND receiver_username = :userA) ORDER BY timestamp DESC LIMIT 1", nativeQuery = true)
	ChatMessage getLastMessage(@Param("userA") String userA, @Param("userB") String userB);
}
